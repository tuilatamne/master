...
function savePhoto()
{
    ...
    if ($id) {
        ...
        if ($d->update('photo', $data)) {
            ...
            // video
            				/* File attach  src mới kh co*/
				if($func->hasFile("file_attach"))
				{
					$fileUpdate_2 = array();
					$file_name_2 = $func->uploadName($_FILES["file_attach"]["name"]);

					if($file_attach_2 = $func->uploadImage("file_attach", $config['photo']['man_photo'][$type]['file_type_photo'], UPLOAD_FILE, $file_name_2))
					{
						$row = $d->rawQueryOne("select id, file_attach from #_photo where type = ? limit 0,1",array($type));
						...
						$fileUpdate_2['file_attach'] = $file_attach_2;
						$d->where('type', $type);
						$d->update('photo', $fileUpdate_2);
						unset($fileUpdate_2);
					}
				}
        ...
        if ($numberPhoto && $dataMultiTemp) {
            ...
            for ($i = 0; $i < count($dataMultiTemp); $i++) {
                ...
                // video
                if (isset($config['photo']['man_photo'][$type]['file_photo']) && $config['photo']['man_photo'][$type]['file_photo'] == true) {
                ...
                if (($success_photo || $success_video || $success_file || !empty($dataMulti['link'])) && empty($response)) {
                    if ($d->insert('photo', $dataMulti)) {
                        ...
                        if ($func->hasFile("file_attach" . $i)) {
                            $fileUpdate_2 = array();
                            $file_name_2 = $func->uploadName($_FILES['file_attach' . $i]["name"]);

                            if ($file_attach_2 = $func->uploadImage("file_attach" . $i, $config['photo']['man_photo'][$type]['file_type_photo'], UPLOAD_FILE, $file_name_2 . $i)) {
                                $fileUpdate_2['file_attach'] = $file_attach_2;
                                $d->where('id', $id_insert);
                                $d->update('photo', $fileUpdate_2);
                                unset($fileUpdate_2);
