        ...
        if ($d->update('product', $data)) {
            ...
            if ($func->hasFile("file1")) {
                ...
                if ($photo1 = $func->uploadImage("file1", $config['product'][$type]['img_type'], UPLOAD_PRODUCT, $file_name1)) {
                    $row1 = $d->rawQueryOne("select id, photo1 from #_product where id = ? and type = ? limit 0,1", array($type));

                    if (!empty($row1)) {
                        $func->deleteFile(UPLOAD_PRODUCT . $row1['photo1']);
                    }
